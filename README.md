...
### Add SASS to the project
   - Follow the instructions in the article to install the `sass` package. [Install Sass](https://sass-lang.com/install/). You need to run the installation script only and verify that the `sass` package appears in the list of dependencies in `package.json`.
   - Add an empty `sass` script to `package.json` scripts.
   - Compose a `sass` script to take all `.scss` files from the `src` folder, compile them to `.css`, and put new file(s) into the `dist/css` folder. You can follow the command from documentation [Sass Basics](https://sass-lang.com/guide/). Please do not remove or change the existing scripts.
   - run `sass` script.
  ...
  ### Style the page
   ...
   - After the `style.scss` file is changed, a new `style.css` file should appear in the `dist/css` folder. Observe how your styles are compiled in CSS.
   ...
   - Observe how your changes are compiled in `dist/css/style.css`.
   #### Use @extend

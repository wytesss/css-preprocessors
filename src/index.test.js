...
describe('CSS Preprocessors', () => {
    ...
    describe('Extend', () => {
        it('should create %block placeholder selector', async() => {
            ...
            return render({ data }).then(output => {
                expect(normalizeStringForTest(output.css.toString().trim())).toBe(".foo { display: flex; flex-direction: column; justify-content: space-between; align-items: center; margin: 20px; border: 2px solid #cccccc; background: #ffffff; border-radius: 5px; }");
            });
        });

...
const {
    normalizeStringForTest,
} = require("../test-utils/normalizeStringForTest");
...
describe("CSS Preprocessors", () => {
    ...
    describe("Sass to CSS convertion", () => {
        ...
        it("should create css files", async () => {
            const isFileCreated = await checkIfFileExist("dist/css/style.css");
    ...
    describe("Variables creation", () => {
        it("should replace _variables.css with _variables.scss", async () => {
            ...
            const isSassFileCreated = await checkIfFileExist(
                "src/_variables.scss"
            );
            expect(isCSSFileExist).toBe(false);
            expect(isSassFileCreated).toBe(true);
        ...
        it("should refactor CSS variables to use Sass approach", async () => {
            ...
            expect(
                normalizedFile.includes("$blue-color: rgba(0, 0, 255, 0.632);")
            ).toBe(true);
            expect(
                normalizedFile.includes("$green-color: rgb(3, 69, 3);")
            ).toBe(true);
            expect(
                normalizedFile.includes("$orange-color: rgb(255, 165, 0);")
            ).toBe(true);
            expect(normalizedFile.includes("$white-color: #ffffff;")).toBe(
                true
            );
            expect(normalizedFile.includes("$grey-color: #cccccc;")).toBe(true);
            expect(
                normalizedFile.includes(
                    "$light-grey-color: rgba(238, 238, 238, 0.933);"
                )
            ).toBe(true);
            expect(normalizedFile.includes("$elem-padding: 0.5rem;")).toBe(
                true
            );
        ...
        it("should use Sass variables in style.scss", async () => {
            ...
            expect(normalizedFile.includes("$white-color")).toBe(true);
        ...
        it("should be converted in CSS properly", async () => {
            ...
            const smallBlockStyleDeclaration = getStyleDeclarationForSelector(
                ".block--small",
                document.styleSheets
            );
            const mediumBlockStyleDeclaration = getStyleDeclarationForSelector(
                ".block--medium",
                document.styleSheets
            );
            const largeBlockStyleDeclaration = getStyleDeclarationForSelector(
                ".block--large",
                document.styleSheets
            );
            const titleGreenStyleDeclaration = getStyleDeclarationForSelector(
                ".block__title--green",
                document.styleSheets
            );
            const titleBlueStyleDeclaration = getStyleDeclarationForSelector(
                ".block__title--blue",
                document.styleSheets
            );
            const titleOrangeStyleDeclaration = getStyleDeclarationForSelector(
                ".block__title--orange",
                document.styleSheets
            );
            const descriptionStyleDeclaration = getStyleDeclarationForSelector(
                ".block__description",
                document.styleSheets
            );
            const notesStyleDeclaration = getStyleDeclarationForSelector(
                ".block__notes",
                document.styleSheets
            );
            ...
            expect(titleGreenStyleDeclaration["background"]).toBe(
                "rgb(3, 69, 3)"
            );
            expect(titleBlueStyleDeclaration["background"]).toBe(
                "rgba(0, 0, 255, 0.632)"
            );
            expect(titleOrangeStyleDeclaration["background"]).toBe(
                "rgb(255, 165, 0)"
            );
    ...
    describe("Mixins", () => {
        it("should create flexible-box mixin with proper rules", async () => {
            const data = `
            @use "src/_mixins" as mixins;
        
            .foo {
            @include mixins.flexible-box(row, space-between, flex-start);
            }
        `;
            ...
            expect(
                normalizeStringForTest(
                    result.css.toString().trim().replace(/\s+/g, " ")
                )
            ).toBe(
                ".foo { display: flex; flex-direction: row; justify-content: space-between; align-items: flex-start; }"
            );
        });

        it("should use mixin in style.scss properly", async () => {
            const styleFile = await readTextFile("src/style.scss");
            const placeholderFile = await readTextFile(
                "src/_placeholders.scss"
            );
            const isMixinsUsedForMain =
                styleFile.includes(
                    "@include flexible-box(row, space-between, flex-start);"
                ) ||
                styleFile.includes(
                    "@include mixins.flexible-box(row, space-between, flex-start);"
                );
            const isMixinsUsedForBlock =
                placeholderFile.includes(
                    "@include flexible-box(column, space-between, center);"
                ) ||
                placeholderFile.includes(
                    "@include mixins.flexible-box(column, space-between, center);"
                );
        ...
        it("should be converted to CSS properly", async () => {
            await waitBrowserLoadEvent(document);
            const mainStyleDeclaration = getStyleDeclarationForSelector(
                "main",
                document.styleSheets
            );
            const blockStyleDeclaration = getStyleDeclarationForSelector(
                ".block--small, .block--medium, .block--large",
                document.styleSheets
            );

            expect(mainStyleDeclaration["display"]).toBe("flex");
            expect(mainStyleDeclaration["flex-direction"]).toBe("row");
            expect(mainStyleDeclaration["justify-content"]).toBe(
                "space-between"
            );
            ...
            expect(blockStyleDeclaration["justify-content"]).toBe(
                "space-between"
            );
    ...
    describe("Functions", () => {
        it("multiply function should multiply 2 numbers", () => {
            const data = `
                @use "src/_functions.scss" as functions;

                .foo {
                    width: functions.multiply(10px, 3);
                }
            `;
        ...
        it("sum function should count sum of 4 numbers", () => {
            const data = `
                ...
                .foo {
                    width: functions.sum(10px, 3px, 2px, 10px);
                }
            `;
        ...
        it("sum function should count sum of 10 numbers", () => {
            const data = `
                ...
                .foo {
                    width: functions.sum(10px, 3px, 2px, 10px, 1px, 12px, 2px, 8px, 9px, 1px);
                }
            `;
        ...
        it("should use functions in style.scss", async () => {
            const styleFile = await readTextFile("src/style.scss");
            const isFunctionUsedProperly =
                styleFile.includes(
                    "width: sum($small, $medium, $large, multiply(6, $block-margin));"
                ) ||
                styleFile.includes(
                    "width: functions.sum($small, $medium, $large, functions.multiply(6, $block-margin));"
                );
    ...
    describe("Extend", () => {
        it("should create %block placeholder selector", async () => {
            const data = `
                @use "src/_placeholders.scss" as *;
            ...
            expect(
                ...
                ".foo { display: flex; flex-direction: column; justify-content: space-between; align-items: center; margin: 20px; border: 2px solid #cccccc; background: #ffffff; border-radius: 5px; }"
            );
        });
        ...
        it("should apply styles properly", async () => {
            ...
            expect(styleDeclaration["background"].toLowerCase()).toBe(
                "#ffffff"
            );
            expect(styleDeclaration["border-radius"]).toBe("5px");
            expect(styleDeclaration["border"].toLowerCase()).toBe(
                "2px solid #cccccc"
            );
